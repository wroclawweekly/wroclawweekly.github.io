...
HTMLArea.Config = function () {
	...
	this.fontname = {
		...
		"Courier New":	   'Andale Mono, Courier New, Courier, monospace',
...
HTMLArea.prototype.generate = function () {
	...
	if (textarea.form) {
		...
		f.onsubmit = function() {
			...
			editor._textArea.value = editor._textArea.value.replace(/<br ?\/?>$/g,"");
			editor._textArea.value = editor._textArea.value.replace(/<br ?\/?>/g,"\n");
	...
	function initIframe() {
		...
		editor._textArea.value = editor._textArea.value.replace(/^\n/g,"");
		editor._textArea.value = editor._textArea.value.replace(/^\r/g,"");
		editor._textArea.value = editor._textArea.value.replace(/\r\n/g,"<br />");
		editor._textArea.value = editor._textArea.value.replace(/\n/g,"<br />");
		editor._textArea.value = editor._textArea.value.replace(/\r/g,"<br />");
...
HTMLArea.getHTML = function(root, outputRoot, editor) {
	...
	while (html.match(/span[^>]+(bold|line-through|italic)[^>]+/)) {
		html = html.replace(/<span[^>]+font-weight: bold;.>(.*(?!span).*)<\/span>/,'<strong>$1</strong>');
		html = html.replace(/<span[^>]+text-decoration: line-through;.>(.*(?!span).*)<\/span>/,'<strike>$1</strike>');
		html = html.replace(/<span[^>]+font-style: italic;.>(.*(?!span).*)<\/span>/,'<em>$1</em>');
		html = html.replace(/<span[^>]+text-decoration: underline;.>(.*(?!span).*)<\/span>/,'$1');
	}
	return html;
};
