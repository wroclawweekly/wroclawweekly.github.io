...
Description: Administration of My Custom Widgets.
*/
    // technical elements 
...
/******************/
      ...
      if ($_POST[$element_widget_backup]==$button_text_set_backup) {
        if ( $_POST[ $data_widget_backup ] != $data_widget_backup ) {
          ...
          $max = MCW_get_max_widgets($cache_all_widgets);
...
/*** fill cache data ***/
/***********************/ 
      $cache_widget = array('name'       => trim($_POST[$data_widget_name]),
                            ...
                            //, 
                            //'aftercode'  => $_POST[$data_widget_code.'_after']);
      $max = MCW_get_max_widgets(MCW_get_all_widgets());
      $help_deleted_flag=0;
      unset($cache_all_widgets);
      for ( $i = 0; $i < $max; ++$i ) {
        //reduce cache count if entry was deleted    
        $help_widget = MCW_get_mywidget($i-$help_deleted_flag);
        $help_widget = array('name'       => $help_widget['name'],
                             ...
                             //, 
                             //'aftercode'  => $_POST[$data_widget_code.$i.'_after']);                           
/**********************/    
/*** delete entries ***/
/**********************/ 
        if ($_POST[$element_widget_submit.$i]==$button_text_delete_single) {
          MCW_delete_mywidget($i);
          $help_deleted_flag++;
...
/*** save single entry ***/
/*************************/
            MCW_set_mywidget($i,$help_widget);
            ?>
            <div id="message" class="updated fade">
...
/**********************/
      ...
      if ($_POST[$element_widget_backup]==$button_text_get_backup){
        if ( $_POST[ $data_widget_backup ] != $data_widget_backup ) {
          ...
          $max = MCW_get_max_widgets($cache_all_widgets);
          echo '<div id="message" class="updated fade"><p>'.$max.' Widgets restored from Backup.</p></div>';
        }
      }
...
/************************/
      if( $_POST[ $element_widget_submit ] == $button_text_new_single ) {
        if ($cache_widget['name'] == ""){
          ...
          if (MCW_widget_allready_exist($cache_widget['name'])){
            echo '<div class="error"><p><b>Widget-ID allready exist!</b></p></div>';
          } else {
            MCW_add_mywidget($cache_widget);
            ...
            $help_plausi_failed=MCW_code_plausibility_check($cache_widget['code'],$cache_widget['kind'] );
...
if (empty($cache_widget)){
  ...
  $cache_widget['kind'] = MCW_std_kind();
}	
...
<?php } ?>
  ...
  <div class="wrap">
    ...
    <fieldset class="options">
      <legend> My Custom Widgets</legend>
        ...
        <table class="mcw_option_table" width="100%"  border="1" cellspacing="3" cellpadding="3">
          ...
          ?>
                ...
                <?php } ?>
                  ...
                  <?php $help_plausi_failed=MCW_code_plausibility_check($help_widget['code'], $help_widget['kind'] ); ?>
                  <?php 
                  if ($help_plausi_failed){
                    echo '<div id="'.$element_widget_warning.$i.'" class="error" onClick="Effect.toggle(\''.$element_widget_container.$i.'\', \'slide\', {duration:0.5});return true;"><p><b>Code did not pass the plausibility check. Please review your code!</b> <br><i>('.$help_plausi_failed.')</i></p></div>';
                  } 
                  ?>
          ...
          <tr valign="center">
            <th align="left" width="0%">
              <input class="input" type="text" name="<?php echo $data_widget_name; ?>" value="<?php echo $cache_widget['name']; ?>" size="20">
            </th>
            <?php if ($javascript_is_allowed){ ?>
              <th align="left" width="0%">
                <a href="#" title="Edit Widget">
            ...
            <tr>
              ...
              <td align="left" width="80%">
                ...
                <div id="<?php echo $element_help_text_backup; ?>" style="display:none;">
                  <br><div id="message" class="updated"><p><b><?php echo $description_backup; ?></b></p></div>
                </div>
              </td>
              <?php if ($javascript_is_allowed){ ?>
            ...
            <tr>
              ...
              <td align="left" width="80%">&nbsp;<br>
                <div id="<?php echo $element_help_text_debug; ?>" style="display:none;">
                  <div id="message" class="updated"><b><?php echo $description_debug; ?></b></div>
                </div>
              </td>
              <?php if ($javascript_is_allowed){ ?>
                <td valign="center" align="center" width="0%">
    ...
    <div width="100%" align="right">
      for plugin options <a href="<?php echo get_option('siteurl'); ?>/wp-admin/options-general.php?page=<?php echo MCW_get_mainfile_name(); ?>">click here &raquo;</a>
    </div>
  </div> 
    ...
    for ( $i = 0; $i < $max; ++$i ) {
      if ($_POST[$element_widget_submit.$i]==$button_text_preview_single) {
        if ($_POST[$data_debug_activate]==$data_debug_activate){
          $args = array('i' => $i, 'force_eval' => true, 'debug_mode' => true);
        } else {
          $args = array('i' => $i, 'force_eval' => true, 'debug_mode' => false);
        }
